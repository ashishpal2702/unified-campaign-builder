...
-- SELECT
CREATE POLICY "Anon can view public campaigns"
ON public.campaigns
FOR SELECT
USING (auth.uid() IS NULL AND user_id IS NULL);

-- INSERT
CREATE POLICY "Anon can create public campaigns"
ON public.campaigns
FOR INSERT
WITH CHECK (auth.uid() IS NULL AND user_id IS NULL);

-- UPDATE
CREATE POLICY "Anon can update public campaigns"
ON public.campaigns
FOR UPDATE
USING (auth.uid() IS NULL AND user_id IS NULL);

-- DELETE
CREATE POLICY "Anon can delete public campaigns"
ON public.campaigns
FOR DELETE
USING (auth.uid() IS NULL AND user_id IS NULL);